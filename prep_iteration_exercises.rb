  ...
  def bubble_sort!(&prc)
    ...
    while !sorted
      ...
      (0...self.length - 1).each do |i| 
        if prc.call(self[i], self[i + 1]) == -1 # we put 1 if we want to sort in descendint order
  ...
  def bubble_sort(&prc)
    ...
    while !sorted
      ...
      (0...ar.length - 1).each do |i| 
        if prc.call(ar[i], ar[i + 1]) == -1
          ar[i], ar[i + 1] = ar[i + 1], ar[i]
          sorted = false
        end
      end
    end
    ar # we can verify that it is another instance with .object_id
  end
end
...
def substrings(string)
  ...
  (0...string.length).each do |i|
    (i...string.length).each do |y|
      array_of_substrings << string[i..y]
    end
  end
  array_of_substrings
end
